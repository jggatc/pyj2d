...
To create a deployable Java app, copy App.py to the script folder with PyJ2D on the path, and edit App.py to import your Python script, set app size, and code to link the app thread to the script application, including script setup, and script execution statements that will update during the app thread loop. Alternatively, use App.py as a guide and edit your Python script accordingly. The code in App.py is public domain and can be used or modified without restrictions. To test, the edited App.py script can be run directly on the desktop JVM, which executes the app code with the pawt module in main(). To create an app jar that can be deployed online, use jythonc available for Jython 2.2.1, with the command 'jythonc --core --deep --jar Pyj2d_App.jar App.py' to package together with Jython dependencies, or 'jythonc --jar Pyj2d_App.jar App.py' to package alone in which case jython.jar must be included. The mixer function requires Mixer.class (compiled with 'javac Mixer.java') and should be included in the jar with 'jar uvf Pyj2d_App.jar pyj2d/Mixer.class'. To update the app jar with a resources folder containing image and audio files, use command 'jar uvf Pyj2d_App.jar resources'. Note that Java apps do not start from main() rather launch from japplet subclass with the same name as the script.
