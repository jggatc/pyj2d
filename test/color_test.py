...
def test_color_constructor():
    ...
    assert c == (0, 255, 0, 255)
    assert (c.r, c.g, c.b, c.a) == (0, 255, 0, 255)
    if env['platform'] in ('jvm', 'js'):
        #pg: integer is rgba, not argb
        c = pg.Color((0xff<<24)+255)
        assert c == (0, 0, 255, 255)
    c = pg.Color((255, 0, 0))
    assert c == (255, 0, 0, 255)
    c = pg.Color((0, 255, 0, 255))
    assert c == (0, 255, 0, 255)
...
def test_color_operator():
    ...
    assert c1 + c2 == (100, 255, 160, 255)
    assert c1 + c3 == (255, 220, 120, 200)
    assert c1 - c2 == (20, 180, 80, 0)
    assert c1 - c3 == (0, 220, 120, 0)
    assert c1 * c2 == (255, 255, 255, 255)
    assert c1 * c3 == (255, 0, 0, 255)
    assert c1 // c2 == (1, 5, 3, 0)
    assert c1 // c3 == (0, 0, 0, 1)
    assert c1 % c2 == (20, 20, 0, 100)
    assert c1 % c3 == (60, 0, 0, 0)
    c = pg.Color(0, 10, 20, 255)
    c += c1
    assert c == (60, 230, 140, 255)
    c = pg.Color(0, 10, 20, 255)
    c -= c1
    assert c == (0, 0, 0, 155)
    c = pg.Color(0, 10, 20, 255)
    c *= c1
    assert c == (0, 255, 255, 255)
    c = pg.Color(0, 10, 20, 255)
    ...
    if not env['platform'] == 'js':    
        #ts error with ~
        assert ~c == (255, 245, 235, 0)
    else:
        assert c.__invert__() == (255, 245, 235, 0)
...
def test_color_transform():
    ...
    c = pg.Color(0, 10, 100, 100)
    assert c.premul_alpha() == (0, 4, 39, 100)
    c = pg.Color(255, 0, 100, 150)
    assert c.premul_alpha() == (150, 0, 59, 150)
    c = pg.Color(0, 10, 100, 100)
    assert c.correct_gamma(0.1) == (0, 184, 232, 232)
    assert c.correct_gamma(0.9) == (0, 14, 110, 110)
    assert c.correct_gamma(2.0) == (0, 0, 39, 39)
    c = pg.Color(255, 0, 100, 150)
    assert c.correct_gamma(0.1) == (255, 0, 232, 242)
    assert c.correct_gamma(0.9) == (255, 0, 110, 158)
    assert c.correct_gamma(2.0) == (255, 0, 39, 88)
    c1 = pg.Color(0, 10, 100, 100)
    c2 = pg.Color(0, 20, 200, 100)
    assert c1.lerp(c2, 0.0) == (0, 10, 100, 100)
    assert c1.lerp(c2, 0.3) == (0, 13, 130, 100)
    assert c1.lerp(c2, 0.5) == (0, 15, 150, 100)
    assert c1.lerp(c2, 0.7) == (0, 17, 170, 100)
    assert c1.lerp(c2, 1.0) == (0, 20, 200, 100)
    c1 = pg.Color(50, 100, 150, 100)
    c2 = pg.Color(100,200,250,200)
    assert c1.lerp(c2, 0.0) == (50, 100, 150, 100)
    assert c1.lerp(c2, 0.3) == (65, 130, 180, 130)
    assert c1.lerp(c2, 0.5) == (75, 150, 200, 150)
    assert c1.lerp(c2, 0.7) == (85, 170, 220, 170)
    assert c1.lerp(c2, 1.0) == (100, 200, 250, 200)
    c1 = pg.Color(255, 0, 100, 150)
    c2 = (50, 100, 150, 200)
    if not env['platform'] == 'js':
        #ts conforms to python round 0.5 to even
        assert c1.lerp(c2, 0.5) == (153, 50, 125, 175)
    else:
        assert c1.lerp(c2, 0.5) == (152, 50, 125, 175)
    c1 = pg.Color(255, 0, 100, 150)
    c2 = (50, 100, 150)
    if not env['platform'] == 'js':
        #ts conforms to python round 0.5 to even
        assert c1.lerp(c2, 0.5) == (153, 50, 125, 203)
    else:
        assert c1.lerp(c2, 0.5) == (152, 50, 125, 202)
...
def test_color_conversion():
    ...
    assert c == (229, 204, 178, 255)
    c.cmy = (0.5, 0.5, 0.5)
    _c, _m, _y = c.cmy
    assert (_rd( _c, _m, _y )) == (0.502, 0.502, 0.502)
    assert c == (127, 127, 127, 255)
    c.cmy = (0.0, 0.5, 1.0)
    _c, _m, _y = c.cmy
    assert (_rd( _c, _m, _y )) == (0.0, 0.502, 1.0)
    assert c == (255, 127, 0, 255)
    c = pg.Color(255, 0, 0)
    assert c.hsva == (0.0, 100.0, 100.0, 100.0)
